...
--TASK5
SELECT ATTRACT_NO, ROUND(HOUR_RATE,1),ROUND(HOUR_RATE + (HOUR_RATE * 0.2),1) 
INCREMENT FROM HOURS;

--TASK6
--In Operator is used for the fetching the data of multiple values from the given table. 
--Example Fetching the f_name and L_name start with the A.
--Between Operator is used to fetch the data between the range of two given values.
--Example Range between 60 and 80.

--TASK7
SELECT * FROM HOURS GROUP BY EMP_NUM;
